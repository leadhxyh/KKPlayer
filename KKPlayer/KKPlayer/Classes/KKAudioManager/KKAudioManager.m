...
typedef struct{
    AUGraph graph;
    KKAudioNodeContext converterNodeContext;
    KKAudioNodeContext mixerNodeContext;
    KKAudioNodeContext outputNodeContext;
    AudioStreamBasicDescription commonFormat;
}KKAudioOutputContext;
...
@property(nonatomic,assign)KKAudioOutputContext *outputContext;
...
- (BOOL)setupAudioUnit{
    ...
    self.outputContext = (KKAudioOutputContext *)malloc(sizeof(KKAudioOutputContext));
    memset(self.outputContext, 0, sizeof(KKAudioOutputContext));
...
- (void)delegateErrorCallback{
    if (self.error) {
        KKPlayerLog(@"KKAudioManager did error : %@", self.error);
    }
}

- (void)delegateWarningCallback{
    if (self.warning) {
        KKPlayerLog(@"KKAudioManager did warning : %@", self.warning);
    }
}
